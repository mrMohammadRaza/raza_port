...
export default function Sidebar({ unreadMessagesCount = 0 }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className="lg:hidden bg-slate-950 border-b border-slate-800 p-4 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-indigo-600 to-sky-500 p-0.5">
      ...
      {/* Sidebar Container (Desktop Sidebar + Mobile Drawer Overlay) */}
      <aside
        className={`fixed lg:sticky top-0 bottom-0 left-0 z-50 lg:z-30 w-64 bg-slate-950 border-r border-slate-800/80 flex flex-col justify-between h-screen transition-transform duration-300 ease-in-out shrink-0 ${
        ...
        <div>
          ...
          <div className="p-4 border-b border-slate-800/60">
            <Link
              ...
              target="_blank"
              onClick={() => setMobileDrawerOpen(false)}
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold text-indigo-400 hover:text-white hover:border-indigo-500/50 transition-all"
            >
